...
const TEMPLATE = readFileSync(join(dirname(fileURLToPath(import.meta.url)), "basket-coin-template.b64"), "utf8").trim();
...
function build(body) {
  ...
  if (u8.length < 64 || u8[0] !== 0xa1 || u8[1] !== 0x1c || u8[2] !== 0xeb || u8[3] !== 0x0b) {
    throw new Error("Patched basket coin bytecode is not a valid Move module");
  }
  return {
...
export { build, uniquifyVecs, DEFAULTS, TEMPLATE };
