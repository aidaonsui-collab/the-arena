...
/// Dynamic-field key on BluefinPositionLock: launch-locked basket-yield mode.
...
public(package) fun collect_lp_fees_return_token<A, B>(
    ...
    assert!(!is_basket_yield(lock), errors::use_basket_yield_collect());
...
public(package) fun attach_holder_yield(lock: &mut BluefinPositionLock, yield_id: ID) {
    ...
    assert!(!is_basket_yield(lock), errors::yield_mode_conflict());
...
public fun is_basket_yield(lock: &BluefinPositionLock): bool {
    df::exists(&lock.id, BasketYieldKey {})
}

public fun basket_yield_id(lock: &BluefinPositionLock): ID {
    assert!(is_basket_yield(lock), errors::not_basket_yield());
    *df::borrow(&lock.id, BasketYieldKey {})
}

public(package) fun attach_basket_yield(lock: &mut BluefinPositionLock, basket_id: ID) {
    assert!(!is_basket_yield(lock), errors::already_locked());
...
/// Collect LP fees and route the pit-bps quote slice into `BasketYieldVault`
/// quote staging instead of the pit pot. Creator/platform unchanged; token A returned for burn.
public(package) fun collect_lp_fees_return_token_to_basket<A, B>(
    lock: &mut BluefinPositionLock,
    ...
    assert!(is_basket_yield(lock), errors::not_basket_yield());
    assert!(!is_holder_yield(lock), errors::yield_mode_conflict());
    basket_yield::assert_bound_to_lock(vault, object::id(lock));
...
public(package) fun abort_basket_yield_collect() {
    abort errors::use_basket_yield_collect()
}
...
#[test_only]
public fun share_bluefin_lock_with_basket_for_testing(
    pool_id: ID,
    bluefin_pool_id: ID,
    beneficiary: address,
    unlock_ms: u64,
...
#[test_only]
public fun debug_attach_basket_for_testing(lock: &mut BluefinPositionLock, basket_id: ID) {
    attach_basket_yield(lock, basket_id);
}

#[test_only]
public fun debug_attach_holder_for_testing(lock: &mut BluefinPositionLock, yield_id: ID) {
    attach_holder_yield(lock, yield_id);
}
