...
public fun buy<T, Q>(
    ...
    config::assert_official_pit_if_registered(config, pit);
...
public fun settle_pit<T, Q>(pool: &mut Pool<T, Q>, pit: &mut Pit<Q>, ctx: &mut TxContext) {
    ...
    if (pool.pit_mode == PIT_HOLDERS) {
        ...
        // Once the curve reserves are vaulted / empty there is nothing to buy
        // against — forfeit so the next `ring_pit` can run. Pot stays.
...
/// Marks the pit settled so the next `ring_pit` can run. Pot stays.
