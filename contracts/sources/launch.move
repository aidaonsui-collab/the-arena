...
/// (fail before creating an orphan vault), vault creation, DF attach, event.
fun migrate_instant_to_holder_yield_inner<T, Q>(
...
/// the same limitation the existing Bluefin collect path already has (no
...
/// Shared body for `migrate_instant_to_basket_yield` and its test-only twin.
fun migrate_instant_to_basket_yield_inner<T, Q>(
    lock: &mut BluefinPositionLock,
    basket: BasketConfig,
    ctx: &mut TxContext,
): ID {
