...
`tsx src/cli.ts perp` refreshes AMC vault NAV into `/api/perp-tokens` (blob-only GET). With `ARENA_PERP_TRADE=1` (default on the Air tick) it longs AMC-USDC on Vice-owned vaults that have idle USDC and no AMC book, and processes vault withdraws. Create/deposit is still signed in the browser.
